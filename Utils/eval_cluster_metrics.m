function score = eval_cluster_metrics(label, true_label)
% Open Source: https://github.com/youweiliang/Multi-view_Clustering

% make sure both labels and true_labels start from 1
label = label + (1 - min(label));
true_label = true_label + (1 - min(true_label));

result = ClusteringMeasure(true_label,label);
ACC = result(1);
PUR = result(3);
NMI = NMImax(label,true_label);
ARI = rand_index(label,true_label,'adjusted');

score = [ACC, NMI, PUR, ARI];
end
...
%   correlation.
%
%   Source: https://github.com/cmccomb/rand_index
